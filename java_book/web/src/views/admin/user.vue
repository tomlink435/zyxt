      ...
      <a-table size="middle" rowKey="id" :loading="data.loading" :columns="columns" :data-source="data.userList"
        ...
        <template #bodyCell="{ text, record, index, column }">
          ...
          <template v-if="column.key === 'operation'">
            <span v-if="userStore.admin_user_id != record.id">
              <a v-if="record.status == 0" @click="handleEdit(record)">启用</a>
              <a v-if="record.status == 1" @click="handleEdit(record)">禁用</a>
        ...
        <div>
          <a-form ref="myform" :label-col="{ style: { width: '80px' } }" :model="modal.form" :rules="modal.rules">
            <a-row :gutter="24">
              ...
              <a-col span="24" v-if="!modal.editFlag">
                <a-form-item label="再输入密码" name="rePassword">
                  <a-input placeholder="请输入" type="password" v-model:value="modal.form.rePassword" allowClear />
                </a-form-item>
              </a-col>
              <!-- <a-col span="24">
              ...
              <a-col span="24">
                <a-form-item label="角色" name="role">
                  <a-select placeholder="请选择" allowClear v-model:value="modal.form.role">
                    <a-select-option key="1" value="1">普通管理员</a-select-option>
                    <a-select-option key="2" value="2">超级管理员</a-select-option>
                  </a-select>
                </a-form-item>
              </a-col>
              <!-- <a-col span="24">
...
import { createApi, listApi, updateApi, deleteApi, startOrStopApi } from '/@/api/admin';
...
const columns = reactive([
  ...
  // },
  {
    ...
    key: 'role',
    align: 'center',
    customRender: ({ text, record, index, column }) => (text == '1' ? '超级管理员' : '普通管理员'),
  },
  {
    title: '状态',
    dataIndex: 'status',
...
const modal = reactive({
  ...
  rules: {
    ...
    rePassword: [{ required: true, message: '请输入', trigger: 'change' }],
...
const handleOk = () => {
  myform.value
    ...
    .then(() => {
      ...
      if (modal.form.rePassword) {
        formData.append('rePassword', modal.form.rePassword);
      }
